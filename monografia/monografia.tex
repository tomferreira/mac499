...
Um cuidado especial na utilização desse algoritmo é na escolha do número de grupos desejado. Uma escolha inadequada poderá resultar em um mau agrupamento, unindo duas classes naturais em um único grupo ou dividindo uma classe natural em dois grupos, porém algumas técnicas tem sido desenvolvidas para resolver esse problema [2, 3, ?]. (colocar mais referências a artigos para solução desse problema) Um exemplo de problema que pode ocorrer está ilustrado pela figura \ref{fig:problema_kmeans}.
...
O sistema hVINA foi desenvolvido utilizando o \textit{framework} Rails [?] e outras tecnologias Web, como a marcação HTML, Javascript e CSS3. Todo o código-fonte segue junto a esta monografia.
...
O algoritmo de tokenização implementado inicialmente na biblioteca é o \textit{Treebank tokenizer} [12], porém a extensão para outros algoritmos de tokenização é possível devido à modularidade da biblioteca.
...
Continuando com o exemplo da figura ?, com a remoção dos \textit{stopwords} o resultado seria da seguinte forma:
...
\textit{Stemming} é o processo de unificar as formas variantes de uma palavra em uma representação comum, chamado de \textit{stem} [4]. Por exemplo, as palavras “corredor”, “corrida” e “correria” podem ser reduzidas para a representação “corr”.
...
A maioria dos algoritmos para \textit{stemming} baseia-se no método de remoção de afixos (sufixos e prefixos), definidos a partir de um conjunto de regras predefinidas. Caso a palavra contenha o prefixo ou sufixo definido por alguma regra, ele é removido [5].
...
\begin{figure}[H]
    ...
    \end{center}
    \caption{Sequência de passos do algoritmo RSLP, retirado de [4].}
    \label{fig:rslp}
\end{figure}
