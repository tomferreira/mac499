...
\begin{equation} P = \frac{VP} {VP + FP} \end{equation}
...
\begin{equation} R = \frac{VP} {VP + FN} \end{equation}
...
\label {sec:testes_tempo_processamento}

Para avaliar a eficiência da biblioteca, foi utilizado um conjunto de 413 artigos do escritor e jornalista José de Paiva Netto \cite{paivanetto}, todos escritos em português.

Foram avaliados o tempo médio de execução em cada etapa e a quantidade de tokens encontrados. Como parâmetros do algoritmo FIHC foram utilizados os seguintes valores sugeridos pelos próprios autores do algoritmo \cite{Ester03} e que também são usados por padrão no sistema hVINA: \textit{global support} = 0,25; \textit{cluster support} = 0,3 e \textit{k}-grupos = 30.

O resultado da tabela \ref{table:experimento} mostra o tempo médio a partir de 10 (dez) execuções\footnote[1]{Os testes foram feitos com o Ruby MRI 2.2.0 e Suse Linux 11 quad-core de 3,2 GHz.} da biblioteca, tendo como entrada o conjunto de artigos descrito acima.
...
\hline
& Deteção & Tokenização & Limpeza & \textit{Stemming} & Agrupamento & Total \\
\hline
Tempo médio & 6,57 & 0,56 & 2,65 & 7,61 & 84,17 & 101,57 \\
Desvio padrão & 0,55 & 0,01 & 0,07 & 0,32 & 1,61 & 1,15 \\
\hline
\end{tabular}
\caption{Tempo médio de execução, em segundos, de cada etapa da biblioteca.}
\label{table:experimento}
\end{table}
...
\subsubsection {Tokenização e limpeza}
\label {sec:testes_tokenizacao_limpeza}

As etapas de tokenização e limpeza são mostradas na tabela \ref{table:experimento_tokens}, que apresenta a quantidade de tokens encontrados pelo algoritmo de tokenização e, desse conjunto, a quantidade removida na etapa de limpeza (remoção dos \textit{stopwords}). A coluna de redução representa proporcionalmente a quantidade de tokens restantes em relação ao conjunto inicial encontrado.
...
\hline
\end{tabular}
\caption{Resultados das etapas de tokenização e limpeza.}
\label{table:experimento_tokens}
\end{table}
...
\bibitem{paivanetto}
    http://www.paivanetto.com.br/index.php/pt/artigos
